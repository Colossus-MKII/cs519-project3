...
/*
 * CS519 Project 3 Part A: Q5 - Data structures for runqueues
 * The main structures managing runqueues are:
 * 1. `struct rq`: The per-CPU runqueue containing locks and sub-runqueues.
 * 2. `struct cfs_rq`: The CFS specific runqueue holding the Red-Black tree root.
 * 3. `struct sched_entity` (defined below): Embedded in task_struct, holds 
 * the vruntime, weight (load), and the rb_node for tree insertion.
 */
struct sched_entity {
	/* For load-balancing: */
