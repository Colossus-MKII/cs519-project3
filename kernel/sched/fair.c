...
/*
 * ============================================================================
 * CS519 Project 3: Part A - CFS Failure to Capture Application Idleness
 * ============================================================================
 * While CFS perfectly distributes CPU time fairly among runnable tasks based 
 * on their vruntime and weights, it suffers from a semantic blind spot: it 
 * cannot distinguish between a thread doing useful work and a thread actively 
 * spinning. For example, busy-waiting on a spinlock or polling for an event).
 * * In a polling scenario, the application is logically "idle", but to CFS, 
 * the thread is actively consuming CPU cycles. Thus, `update_curr()` will 
 * continuously advance its `vruntime`, and CFS will treat it as a heavy 
 * CPU consumer. This wastes CPU resources that could have been allocated 
 * to other productive threads. This architectural limitation necessitates 
 * a cooperative mechanism (like our custom system call) where the application 
 * can explicitly inform the scheduler of its inactivity, allowing the OS 
 * to deprioritize it without altering the static nice values.
 * ============================================================================
 */
...
/*
 * CS519 Project 3 Part A: Q6, Q7, Q8 - Nice values and Statistics
 * * Q6 (Nice to Priority): CFS doesn't use strict priorities. Instead, nice values 
 * are mapped to weights (sched_prio_to_weight). Inside this function, delta_vruntime 
 * is calculated inversely proportional to the task's weight. A lower nice value 
 * means a higher weight, causing vruntime to grow slower.
 * * Q7 (Statistics Collection): This function calculates the actual physical time 
 * executed (delta_exec) and updates the sum_exec_runtime in the sched_entity.
 * * Q8 (Exposing to Userspace): These statistics (like sum_exec_runtime, wait times) 
 * are exposed to userspace primarily through the procfs virtual file system, 
 * specifically in /proc/[pid]/sched and /proc/schedstat.
 */
...
/*
 * CS519 Project 3 Part A: Q1 - Adding tasks to a per-CPU runqueue
 * This function handles adding a task to the CFS runqueue. It calls 
 * enqueue_entity() to update the task's scheduling statistics and inserts 
 * the task's sched_entity into the CFS Red-Black tree (rb_tree) sorted by 
...
/*
 * CS519 Project 3 Part A: Q3 - Descheduling tasks (Voluntary)
 * When a task blocks (e.g., waiting for I/O), this function is called.
 * It invokes dequeue_entity() to remove the task's sched_entity from the 
 * CFS Red-Black tree.
 */
...
/*
 * CS519 Project 3 Part A: Q2 - Scheduling tasks on a CPU
 * This function selects the next task to run on the CPU. Under CFS, it calls 
 * pick_next_entity() which consistently picks the leftmost node of the 
 * Red-Black tree (the task with the smallest vruntime, heavily starved of CPU).
 * It then calls set_next_entity() to mark it as the running task and extracts 
 * it from the rb-tree.
 */
 ...
 */
static inline bool allow_numa_imbalance(unsigned int running,
					unsigned int weight)
{
...
/*
 * CS519 Project 3 Part A: Q4 - Runqueue rebalancing across CPUs
 * This function handles load balancing across different CPUs within a 
 * sched_domain. It checks if the workload (task weights) is unevenly 
 * distributed. If an imbalance is found, it pulls tasks from the busiest 
 * CPU's runqueue and attaches them to the idle or less-loaded CPU's rq.
 */
...
/*
 * CS519 Project 3 Part A: Q3 - Preempting tasks (Involuntary)
 * Triggered by the periodic timer interrupt. It calls entity_tick() to check 
 * if the current task has exceeded its ideal timeslice. If it has, it sets 
 * the TIF_NEED_RESCHED flag, signaling the kernel to preempt it.
 */
