...
SYSCALL_DEFINE1(set_inactive, int, flag)
{
	pid_t tgid;
	...
	/* Step 2 Requirement: Collect information specifically the TGID */
	tgid = current->tgid;

	if (flag == 1) {
		printk(KERN_INFO
		       "Cooperative Sched: App (TGID: %d, PID: %d) marked as INACTIVE\n",
		       tgid, current->pid);
	} else {
		printk(KERN_INFO
		       "Cooperative Sched: App (TGID: %d, PID: %d) RE-ACTIVATED\n",
		       tgid, current->pid);
	}

	current->cooperative_inactive = flag;
